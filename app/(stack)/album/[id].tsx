    ...
    if (tracksRes.data) {
      setTracks(
        tracksRes.data.map((t: any) => ({
          ...
          album_title: albumRes.data?.title,
        })),
      );
    }
    setIsLoading(false);
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            >
              <Image
                source={{ uri: album.cover_url || PLACEHOLDER_ARTWORK }}
                style={{
                  width: ARTWORK_SIZE,
                  height: ARTWORK_SIZE,
                  borderRadius: 12,
              ...
              <TouchableOpacity
                onPress={() =>
                  router.push(`/(stack)/artist/${album.artist_id}`)
                }
              >
                <Text
                  style={{
              ...
              >
                {album.release_year} · {tracks.length} tracks · {album.category}
              </Text>
